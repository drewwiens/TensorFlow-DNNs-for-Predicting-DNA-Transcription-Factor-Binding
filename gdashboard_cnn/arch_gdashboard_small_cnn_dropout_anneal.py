...
_datasets = ['wgEncodeAwgTfbsBroadK562CtcfUniPk', # original set
    'wgEncodeAwgTfbsHaibK562Atf3V0416101UniPk',
    'wgEncodeAwgTfbsHaibK562Cebpbsc150V0422111UniPk',
    'wgEncodeAwgTfbsHaibK562CtcfcPcr1xUniPk',
    'wgEncodeAwgTfbsHaibK562E2f6V0416102UniPk',
...
def load_ENCODE_k562_dataset(dataset_num, dataset_type):
    global _index_in_epoch_train    
    global _index_in_epoch_test
    global _index_in_epoch_validation
    global _train_features
...
def main(_):
    ...
    for dataset_num in range(0, len(_datasets)):
        for dataset_type in range(0,2): # 0==motif occupancy, 1==motif discovery
            ...
            conv2 = convolution2d(conv1_drop, n_conv2, [L_conv2,1], padding='VALID', normalizer_fn=None)
            ...
            upper_keep_prob = 0.76
            target_keep_prob = 0.75
            keep_prob_anneal_const = 0.0001
            while stop_condition is None:
                #this_conv_keep_prob += keep_prob_anneal_const * (upper_keep_prob - this_conv_keep_prob)  # prop. type 1
                #this_conv_keep_prob = 0.5+math.sqrt(float(i)/320000.0) # keep_prob=0.5..0.75 for i=0..20k  # sqrt type 1
                #this_conv_keep_prob = 0.5+5.6*math.sqrt(float(i)/10000000.0) # keep_prob=0.5..0.75 for i=0..20k  # sqrt type 2
                #this_conv_keep_prob = 0.5+56.0*math.sqrt(float(i)/1000000000.0) # keep_prob=0.5..0.75 for i=0..20k  # sqrt type 3
                this_conv_keep_prob = 0.5+80.0*math.sqrt(float(i)/1000000000.0) # keep_prob=0.5..0.75 for i=0..20k  # sqrt type 4
                #this_conv_keep_prob = 0.5+85.0*math.sqrt(float(i)/1000000000.0) # keep_prob=0.5..0.75 for i=0..20k  # sqrt type 5
                # linear type 1: -----
                #this_conv_keep_prob = 0.5 + 0.000015*float(i)
                #if i > 13000:
                #    this_conv_keep_prob += 0.00005*float(i-13000) # inc rate at very end of training
                #-------
                ...
                if i%1000 == 0:
                    ...
                    while _validation_epochs_completed - prev_validation_epochs_completed == 0: # do in mini batches because single GTX970 has insufficient memory to test all at once
                        ...
                        if pred_validation_labels is None:
                            pred_validation_labels = y_conv_softmax.eval(feed_dict={x: validation_batch[0], y_: validation_batch[1], conv_keep_prob:1.0})
                            true_validation_labels = validation_batch[1]
                        else:
                            pred_validation_labels = numpy.vstack([pred_validation_labels, y_conv_softmax.eval(feed_dict={x: validation_batch[0], y_: validation_batch[1], conv_keep_prob:1.0})])
                    ...
                    print("%s, dataset %d, dataset type %d, epoch %d, step %d, time elapsed %g, validation roc auc %g, perc chg in auc %g, conv_keep_prob %g"%(_datasets[dataset_num], dataset_num, dataset_type, _train_epochs_completed, i, time.time()-t0, roc_auc, perc_chg_auc, this_conv_keep_prob))
                    prev_auc = roc_auc
                    t0 = time.time()
                batch = get_next_batch(0)
                train_step.run(feed_dict={x: batch[0], y_: batch[1], conv_keep_prob:this_conv_keep_prob})
                #check stop condition:
                #if target_keep_prob - this_conv_keep_prob < 0.01:
                if i > 7000:
                    stop_condition = 1
                i += 1
            ...
            while _test_epochs_completed == 0: # do testing in mini batches because single GTX970 has insufficient memory to test all at once
                ...
                if pred_test_labels is None:
                    pred_test_labels = y_conv_softmax.eval(feed_dict={x: test_batch[0], y_: test_batch[1], conv_keep_prob:1.0})
                    true_test_labels = test_batch[1]
                else:
                    pred_test_labels = numpy.vstack([pred_test_labels, y_conv_softmax.eval(feed_dict={x: test_batch[0], y_: test_batch[1], conv_keep_prob:1.0})])
                    true_test_labels = numpy.vstack([true_test_labels, test_batch[1]])
            fpr, tpr, _ = roc_curve(true_test_labels[:,0], pred_test_labels[:,0])
            roc_auc = auc(fpr, tpr)
            print("%s, dataset %d, dataset type %d, final test roc auc %g"%(_datasets[dataset_num], dataset_num, dataset_type, roc_auc))
            f=open(file_name,'a')
            f.write(str(dataset_num) + ',' + str(dataset_type) + ',' + _datasets[dataset_num] + ',' + str(roc_auc) + '\n')
            f.close()

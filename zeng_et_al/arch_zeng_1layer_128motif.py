...
def main(_):
    ...
    for dataset_num in range(0, len(_datasets)):
        ...
        n_conv2 = 768
        L_conv2 = 24
        maxpool_len2 = int(101-L_conv2+1) # global maxpooling (max-pool across temporal domain)
        conv2 = convolution2d(x_image, n_conv2, [L_conv2,1], padding='VALID')
        conv2_pool = max_pool2d(conv2, [maxpool_len2,1], [maxpool_len2,1])

        # FC LAYER
        fc1 = fully_connected(flatten(conv2_pool), 32)
        #fc1_drop = tf.nn.dropout(fc1, 1.0)

        # LINEAR FC LAYER
        y_conv = fully_connected(fc1, 2, activation_fn=None)

        ...
        for motif_occ in range(0,2):
            ...
            if success:
                with tf.variable_scope('scopename_' + str(dataset_num) + '_' + str(motif_occ)):
                    ...
                    # CONVOLUTIONS
                    convs_list_flat = []
                    for L_conv1 in range(3,40,1):  # try conv kernels up to some fraction of the dna sequence length 101
                        n_conv1 = 96
                        conv1 = convolution2d(x_image, n_conv1, [L_conv1,4], padding='VALID', normalizer_fn=None)
                        conv1_drop_len = int(101-L_conv1+1)
                        conv1_pool = max_pool2d(conv1, [conv1_drop_len,1], [conv1_drop_len,1]) # global max-pooling
                    ...
                    while stop_condition is None:
                        ...
                        #    t0 = time.time()
                        if _train_features.shape[0] > 1024*5:
                            batch = get_next_batch(0,1024)
                            stop_steps = 875
                        else:

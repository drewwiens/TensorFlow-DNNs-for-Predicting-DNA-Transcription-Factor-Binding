...
def get_next_batch(dataset_to_use=0, batch_size=2048):
...
def main(_):
    ...
    f.close()
    for dataset_num in range(61, len(_datasets)):
        for motif_occ in range(0,2):
            ...
            if success:
                with tf.variable_scope('scopename_' + str(dataset_num) + '_' + str(motif_occ)):
                    ...
                    n_steps = 101 # lstm timesteps (101 dna bases)
                    ...
                    x_unpacked = tf.unpack(x_image, axis=1) # this func does it all for us :)
                    lstm_fw_cell = rnn_cell.BasicLSTMCell(lstm_n_hidden)
                    lstm_bw_cell = rnn_cell.BasicLSTMCell(lstm_n_hidden)
                    birnn_out,_,_ = tf.nn.bidirectional_rnn(lstm_fw_cell, lstm_bw_cell, x_unpacked, dtype=tf.float32)
                    
                    # Linear activation
                    # rnn_out = birnn_out[-1] # ...using LAST of the rnn inner loops (as in the MNIST example)
                    rnn_out = tf.div(tf.add_n(birnn_out), n_steps) # ...using AVERAGE of the rnn inner loops
                    ...
                    while stop_condition is None:
                        ...
                        if 1 == 0: # turned off
                            ...
                            t0 = time.time()
                        if _train_features.shape[0] > 2048*5:
                            batch = get_next_batch(0)
                            stop_steps = 600
                        else:
                            batch = get_next_batch(0,256)
                            stop_steps = 4800
